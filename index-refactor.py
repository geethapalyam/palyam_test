...
def sendSlackNotification(pr_number):
    # Code to send alert to the slack channel
    url = webhook_url
    message = ("new Pull Request Link: https://github.com/{}/{}/pull/{}".format(username, repoName, pr_number))
...
def sendEmailNotification(pr_number):
    # Code to send alert via email
    smtp_server = "smtp.gmail.com"
    port = 587  # For starttls

    url = "https://github.com/{}/{}/pull/{}".format(username, repoName, pr_number)
    message = '''hello
...
def main():
    ...
    if runnerVersion.group(1) != response.json()['name']:
        ...
        sendSlackNotification(pr.number)

        print("sending notification to the Mail")
        sendEmailNotification(pr.number)
